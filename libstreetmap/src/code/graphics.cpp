...
#define FONT_CACHE_SIZE_FOR_ROTATED 10000
...
event_loop(void (*act_on_mousebutton)(float x, float y, t_event_buttonPressed button_info),
    ...
    // Define AUTO_MARKING to just generate a picture in postscript format
    // of the graphics on the screen and return. Useful for automarking in courses.
#ifdef AUTO_MARKING
...
#define MY_POINT POINT
        ...
        if (npoints > MAX_FIXED_POLY_PTS) {
            transpoints = (MY_POINT *) my_malloc(npoints * sizeof (MY_POINT));
        }
        for (i = 0; i < npoints; i++) {
...
void drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    ...
    float angle = (float) PI * gl_state.currentfontrotation / 180;
...
do_font_loading(int pointsize, float degrees) {
    ...
    if (success == false) {
        ...
        if (degrees != 0) {
            printf("and rotation %f deg", degrees);
        }
